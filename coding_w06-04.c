...
int main() {
    ...
    double x = 100.0;// เริ่มต้น x เป็น 100.0
    int y = 7;  // กำหนดค่า y เป็น 7
    int z = 3;// กำหนดค่า z เป็น 3
    int a = 2;// กำหนดค่า a เป็น 2
    double total = 500.0;// กำหนดค่า total เป็น 500.0
    double price = 100.0;// กำหนดราคาเริ่มต้นเป็น 100.0
    int quantity = 5;// กำหนดจำนวนสินค้าเป็น 5
    double discount = 20.0;// กำหนดส่วนลดเป็น 20.0
    double rate = 5.0; // สมมติ 5% อัตราดอกเบี้ย
    double score = 85.0;// กำหนดคะแนนเริ่มต้นเป็น 85.0
    double penalty = 2.0;// กำหนดค่าปรับเป็น 2.0
    int mistake = 3;// กำหนดจำนวนความผิดพลาดเป็น 3
    ...
    printf("   แบบเต็ม (x = x - 4.0;): x = %.2f\n", x_full_1);// แสดงผลลัพธ์ของ x ในรูปแบบเต็ม

    
    x_short_1 -= 4.0;
    printf("   แบบย่อ (x -= 4.0;):    x = %.2f\n", x_short_1);// แสดงผลลัพธ์ของ x ในรูปแบบย่อ

    if (x_full_1 == x_short_1) {// เปรียบเทียบผลลัพธ์ของทั้งสองแบบ
        printf("   ผลลัพธ์เท่ากัน: True\n\n");// ถ้าเท่ากันจะแสดง True
    } else {
        printf("   ผลลัพธ์เท่ากัน: False\n\n");// ถ้าไม่เท่ากันจะแสดง False
    }

    // ----- Expression ที่ 2: x = 6.5 * x; -----
    printf("--- 2. x = 6.5 * x; ---\n");
    double x_full_2 = x; // ใช้ค่า x ล่าสุดจากการคำนวณก่อนหน้า
    double x_short_2 = x;// สร้างตัวแแปรสำเนาสำหรับแบบย่อ

    
    x_full_2 = 6.5 * x_full_2;
    printf("   แบบเต็ม (x = 6.5 * x;): x = %.2f\n", x_full_2);// แสดงผลลัพธ์ของ x ในรูปแบบเต็ม

    
    x_short_2 *= 6.5; // หรือ x *= 6.5;
    printf("   แบบย่อ (x *= 6.5;):    x = %.2f\n", x_short_2);// แสดงผลลัพธ์ของ x ในรูปแบบย่อ

    if (x_full_2 == x_short_2) {
        printf("   ผลลัพธ์เท่ากัน: True\n\n");// เปรียบเทียบผลลัพธ์ของทั้งสองแบบ
    } else {
        printf("   ผลลัพธ์เท่ากัน: False\n\n");// ถ้าไม่เท่ากันจะแสดง False
    }
    ...
    x_int_full_3 = x_int_full_3 % divisor_3;// คำนวณค่า x แบบเต็ม
    printf("   แบบเต็ม (x = x %% (y + z * a);): x = %d (เมื่อ x เริ่มต้นเป็น %d)\n", x_int_full_3, (int)x);// แสดงผลลัพธ์ของ x ในรูปแบบเต็ม

    
    x_int_short_3 %= divisor_3;// คำนวณค่า x แบบย่อ
    printf("   แบบย่อ (x %%= (y + z * a);):    x = %d\n", x_int_short_3);// แสดงผลลัพธ์ของ x ในรูปแบบย่อ
